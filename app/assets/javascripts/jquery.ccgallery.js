...
jQuery(function($){
	...
	function ccgalleryCreate( $galleryConfig ) {
		...
		for( var i = 0; i < itemnum; i++ ) {
			...
			thumbli += '<img src="'+ $thumbs.eq(i).text() +'" alt="" />';
		...
		if( ie9js ) {
			$coverList.delegate('li', 'mouseleave click', function(e){
				$(this).removeClass('ie7_class7');
			});

			$thumbList.delegate('li', 'mouseleave', function(e){
	...
	function ccgallerySetup( itemnum, $files ) {
		...
		$ccgallery.find('menu').delegate('a.navbuttons', 'click', function(){
			var $this = $(this),
		...
		$thumbList.delegate('img', 'click', function(){
			...
			thumbIndex = $thumbList.find('img').index(this);
		...
		if( canvasYes ) {
			height = (1.5*height) | 0;  // equivalent to Math.floor(1.5*height)
		...
		function setupCoverflow() {
			...
			if( canvasYes ) {
				var i = itemnum, z, left, cwidth, cleft, imageIndex;
				...
				while(i--) {
					...
					coverCanvas[i][0].height = (1.5*coverHeight[i]) | 0;  // equivalent to Math.floor(1.5*coverHeight[i])

					covers[i] = new Plane(coverWidth[i], coverCanvas[i][0].height, 300, coverCtx[i], '#666', bgcolor, $items[imageIndex]);  // the height of the plane takes into account the reflected height also
		...
		$coverContainer.delegate('a.coverclick', 'click', function(){
			var $this = $(this),
		...
		$coverList.delegate('li', 'click', function(){
			var $this = $(this),
				thisindex = $this.index(),
				mode = 'coverflow';

			if( $this.hasClass('active') ) {
				var link = $files.eq(thisindex).find('link').text();
				if( link ) {
					window.location.href = link;
				}
				else {
					overlayCreate( mode, $this.data('type'), $files.eq( $this.data('id') - 1 ).find('source'), $this.find('h2')[0].innerHTML, $this.find('p')[0].innerHTML );
				}
			}
			else {
				if( thisindex !== index ) {
		...
		function flowanimate(newindex) {
			if(newindex >= 0 && newindex < itemnum) {
				...
				if( canvasYes ) {
					...
					var z, left, cwidth, cleft, posZ, angle;
					...
					while(i--) {
						if(i < newindex) {
							...
							z = itemnum;
							left = width/2 - coverCanvas[newindex][0].width/2;
			...
			$mask.css('height', $(document).height()).fadeIn(400, function(){
				...
				if( type === 'photo' ) {
					...
					$image[0].onload = function(){
						...
						overlayShow( $image[0].width, $image[0].height + 80, top);
					};
				}
				else if( type === 'audio') {
					var audio = '<audio controls preload="auto">',
						safariAudio = '<audio controls preload="auto"';
					...
					if( $.browser.safari) {  // Safari (Windows) without Quicktime installed removes <source> tags. This is the fix
					...
					if( $win.width() <= 480 ) {
						overlayShow( $win.width() - 80, 110, top);
					}
					else {
						overlayShow( 400, 110, top);
					}
					...
					if( sources.eq(0).text().indexOf('youtube') !== -1 ) {
						...
						var video = '<video controls preload="auto" width="'+ videoWidth +'" height="'+ videoHeight +'">',
							safariVideo = '<video controls preload="auto" width="'+ videoWidth +'" height="'+ videoHeight +'"';
					...
					overlayShow( videoWidth, videoHeight+80, top);
				}
			});
		...
		$prevItem.click(function(){
			...
			if( mode === 'coverflow' ) {
				...
				$thumbList.find('img').eq(thumbIndex-1).trigger('click');
			}
...
function Plane(width, height, focalLength, ctx, color, bgcolor, img) {
	this.width = width;
	this.height = height;
	...
	this.vertexPoints = [
		make3DPoint(-this.width/2, this.height/2, 0), // top left
		make3DPoint(this.width/2, this.height/2, 0),  // top right
		make3DPoint(this.width/2, -this.height/2, 0),   // bottom right
		make3DPoint(-this.width/2, -this.height/2, 0)   // bottom left
	];
...
Plane.prototype.render = function() {
	...
	this.maxX = (Mathmax(points[0].x, points[1].x, points[2].x, points[3].x) + 1) | 0;  //equivalent to Math.ceil
	this.minX = Mathmin(points[0].x, points[1].x, points[2].x, points[3].x) | 0;  //equivalent to Math.floor
	this.maxY = (Mathmax(points[0].y, points[1].y, points[2].y, points[3].y) + 1) | 0;  //equivalent to Math.ceil
	this.minY = Mathmin(points[0].y, points[1].y, points[2].y, points[3].y) | 0;  //equivalent to Math.floor
	this.maxwidth = this.maxX - this.minX;  //screen width of trapezium
	this.maxheight = this.maxY - this.minY  //screen height of trapezium
	this.maxoffset = ((Mathabs( (points[0].x - 0) - (this.cWidth - points[1].x) )) + 0.5) | 0;  //distance by which the trapezium is offset from canvas centre
};
...
function Transform3DTo2D(points, axisRotations, position, focalLength, centerx, centery) {
	...
	while (i--) {
		...
		y = -(y*scaleFactor) + centery;   //the minus sign makes the y-axis point down, which is the case with computer screens
		//z = z;
